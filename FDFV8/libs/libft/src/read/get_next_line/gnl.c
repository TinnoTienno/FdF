...
/*   Updated: 2024/04/04 23:29:37 by eschussl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_gnl_struct	*gnl_stc_no_static(int fd)
{
	...
	stc = malloc(sizeof(t_gnl_struct));
	if (!stc)
		gnl_flush(NULL, NULL, "Error in GNL function : Malloc\n");
	stc->str = malloc (GNL_BUFFER_SIZE + 1);
	if (!stc->str)
		gnl_flush(NULL, stc, "Error in GNL function : Malloc\n");
	reader = read(fd, &buffer, GNL_BUFFER_SIZE);
	if (reader == -1)
		gnl_flush(NULL, stc, "Error in GNL function : Unread\n");
	stc->str[reader] = '\0';
	stc->size = reader;
	stc->nlpos = -1;
	stc->read = reader;
